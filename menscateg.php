...
<section id="filtersection">
							...
							<h6>Home&nbsp; /&nbsp;<span>Men's Shoes</span></h6>
...
<section id="subcat">
						...
						<?php 			
			$query="select * from menshoescat"; 			
			$run= mysqli_query($conn,$query); 			
			while ($row=mysqli_fetch_array($run)) 			 
				{ 				
					$mshid=$row['mshid']; 				
								
					$mshtitle=$row['mshtitle'];  				
					echo" 			
					<div class='subcat'>
			
					<h1><a href='menscateg.php?mshid=$mshid'>$mshtitle</a></h1>
...
<section id="cat_img">
	<div class="subimage">
		...
		<div class="grid-container">
			...
			$query="select * from images where mshid='$mshid'"; 			
			$run= mysqli_query($conn,$query); 			
			while ($row=mysqli_fetch_array($run)) 			 
				{ 				
					$id=$row['id']; 				
				...
				<?php 	
			if(isset($_GET['mshoeid']))	
			{
				$mshoeid=$_GET['mshoeid'];

			$query="select * from images where mshoeid='$mshoeid'"; 			
			$run= mysqli_query($conn,$query); 			
			while ($row=mysqli_fetch_array($run)) 			 
				{ 				
					$id=$row['id']; 				
...
<div id="size1">
	<h5>Size</h5>
		<div class="squares">
			...
			$query="select * from menshoesize"; 			
			$run= mysqli_query($conn,$query); 			
			while ($row=mysqli_fetch_array($run)) 			 
				{ 				
					$mshoeid=$row['mshoeid']; 				
					$mshoesize=$row['mshoesize'];  				
					echo" 			
					<a href='menscateg.php?mshoeid=$mshoeid'>
					<div class='icon1'>
        			 <p>$mshoesize</p>
      				</div></a>				
					"; 			
